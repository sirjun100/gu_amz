 ...
 * @return {boolean}
 */
function clickPointPressure(x, y, pressure) {
    return agentEvent.clickPointPressure(x, y, pressure);
}
 ...
 * @return {null|string} 字符串，里面的key分别是  orientation, screenWidth, screenHeight
 */
 ...
 * @return {boolean} true 成功，false 失败
 */
function typingText(content) {
    return agentEvent.typingText(content);
}
 ...
 */
function appLaunchEx(bundleId, ignoreState) {
 ...
 * @return {boolean} true 成功，false 失败
 */
function appKillByBundleIdEx(bundleId, ignoreState) {
 ...
 */
function appLaunchByPrefix(bundleIdPrefix) {
    return appHelper.appLaunchByPrefix(bundleIdPrefix);
}
 ...
 * @return {boolean} true 成功，false 失败
 */
function uploadInsertVideo(path) {
    return agentEvent.uploadInsertVideo(path);
}
 ...
 * @return {boolean} true 成功，false 失败
 */
function uploadInsertImage(path) {
    return agentEvent.uploadInsertImage(path);
}
 ...
 */
function setAssistiveTouch(open) {
    return agentEvent.setAssistiveTouch(open);
}
...
function enableDeveloperMode() {
    return agentEvent.enableDeveloperMode();
}
...
function reconnectUsb() {
    return agentEvent.reconnectUsb();
}
 ...
 */
function getPasteboard(contentType) {
    return agentEvent.getPasteboard(contentType);
}
 ...
 * @return {boolean} true 成功，false 失败
 */
function setPasteboard(content, contentType) {
    return agentEvent.setPasteboard(content, contentType);
}


/**
 * 打开代理程序到前台,效果相当于 appLaunch函数
 * 测试过15+正常执行，游戏机和15一下自行测试
 * 支持 EC iOS 6.0.0+
 * @return {boolean} true 成功，false 失败
 */
function takeMeToFront() {
    return agentEvent.takeMeToFront();
}
 ...
 * @return {boolean} true 滑动成功, false 滑动失败
 */
function swipeToPointPressure(startX, startY, endX, endY, duration, pressure) {
    return agentEvent.swipeToPointPressure(startX, startY, endX, endY, duration, pressure);
}
 ...
 * @return {number|AutoImage} 如果是null代表没内容
 */
 ...
 * @return {boolean} true 成功 false 失败
 */
function touchAndHold(x, y, delay) {
    return agentEvent.touchAndHold(x, y, delay);
}
 ...
 * @param value 例如 //value[@value='易点云测'] 代表选取文本等于 易点云测 的节点
 * @return {S} 节点选择器
 */
