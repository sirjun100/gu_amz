...
/**
 * 取得屏幕宽度高度
 * @return {null|string} 例如 750,1334
 */
DeviceWrapper.prototype.getScreenWidthHeightText = function () {
    if (deviceWrapper == null) {
        return;
    }
    return deviceWrapper.getScreenWidthHeightText();
};
 ...
 * 获取当前设备的所有运行信息
 * 适配EC iOS USB 9.20.0+
 * @returns {string} 所有运行信息
 */
DeviceWrapper.prototype.getDeviceMsg = function () {
    if (deviceWrapper == null) {
        return;
    }
    return deviceWrapper.getDeviceMsg();
};
 ...
 * 获取屏幕方向
 * 无需自动化服务
 * 适配EC iOS USB 版本 9.20.0+
 *
 * @returns {*|number} 1 竖屏 2 横屏
 */
DeviceWrapper.prototype.getOrientationNoAuto = function () {
    if (deviceWrapper == null) {
        return 1;
    }
    return deviceWrapper.getOrientationNoAuto();
};
